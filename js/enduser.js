                    ...
                    not_valid_phone_char_msg)
{
    ...
    this.not_valid_phone_char_msg          = not_valid_phone_char_msg;
    ...
    with (fld_data) for (i = 0; (i < fields.length) && fields[i].type; i++)
    {
        ...
        switch (fields[i].type)
        {
            ...
            case 5: // CDT_VARCHAR
                ...
                // Need to utilize the API regex for phone fields
		if (fields[i].name == 'wf_2_100015' || fields[i].name == 'wf_2_100016' || 
		    fields[i].name == 'wf_2_100017' || fields[i].name == 'wf_2_100018' || fields[i].name == 'wf_2_100019')
		{
		    var validInput = new RegExp("^[-A-Za-z0-9,# +.()]+$");

                    if(fld.value && !validInput.test(fld.value))
                    {
                        alert(not_valid_phone_char_msg.replace("%s", fields[i].label));
                        fld.focus();
		        return (false);
                    }
                }
